...
namespace Grand.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        {
            var obj = processPaymentRequest.CustomValues.Where(w => w.Key.ToString().ToUpper() == "DOWNPAYMENTAMOUNT").Select(s => s.Value).FirstOrDefault();
            decimal balance = processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.DOWNPAYMENT" || processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.BANKTRANSFER" ? Math.Round(details.OrderTotal, 6) - Convert.ToDecimal(obj) : Math.Round(details.OrderTotal, 6);
            decimal amountPaid = processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.DOWNPAYMENT" || processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.BANKTRANSFER" ? Convert.ToDecimal(obj) : 0;
            var order = new Order {
        ...
        public virtual async Task<PlaceOrderResult> PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //Payment Collection
                    if(processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.DOWNPAYMENT" || processPaymentRequest.PaymentMethodSystemName.ToUpper() == "PAYMENTS.BANKTRANSFER")
                    {
                        var obj = processPaymentRequest.CustomValues.Where(w => w.Key.ToString().ToUpper() == "DOWNPAYMENTAMOUNT").Select(s => s.Value).FirstOrDefault();
                        var attachment = processPaymentRequest.CustomValues.Where(w => w.Key.ToString().ToUpper() == "ATTACHMENT").Select(s => s.Value).FirstOrDefault();
                        var attachmentFile = processPaymentRequest.CustomValues.Where(w => w.Key.ToString().ToUpper() == "ATTACHMENTFILE").Select(s => s.Value).FirstOrDefault();
                        await _orderService.AddPaymentColection(new PaymentCollections {
                            Active = true,
                            OrderNumber = order.OrderNumber,
                            AmountPaid = Convert.ToDecimal(obj),
