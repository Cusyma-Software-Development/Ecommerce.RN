...
namespace Grand.Web.Controllers
{
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> FailedPayment(string orderId,string errorCode)
        {
            //validation
            if ((_workContext.CurrentCustomer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed))
                return Challenge();
        ...
        protected async Task<JsonResult> OpcLoadStepAfterShippingAddress(IList<ShoppingCartItem> cart)
        {
            ...
            shippingMethodModel.StoreName = _storeContext.CurrentStore.Name;
            
            //shippingMethodModel.ShippingMethods.ForEach(fe =>
            //{
            //    fe.Description = String.IsNullOrEmpty(fe.Description) ? fe.Description : fe.Description.Replace("{StoreName}", _storeContext.CurrentStore.Name);
            //    fe.Name = String.IsNullOrEmpty(fe.Name) ? fe.Name : fe.Name.Replace("{StoreName}", _storeContext.CurrentStore.Name);
            //});
        ...
        [AllowAnonymous]
        [HttpPost]
        public virtual async Task<IActionResult> NotificationRoute()
        {
        ...
        [AllowAnonymous]
        public virtual async Task<IActionResult> RedirectPaymentSuccess(string requestid, string responseid)
        {
            try
            {
                if (!String.IsNullOrEmpty(responseid) && !String.IsNullOrEmpty(requestid))
                {
                    ...
                    using (SHA512 shaM = new SHA512Managed())
                    {
                        ...
                        querySignature = sBuilder.ToString().ToLowerInvariant();
                    }
                    HttpClient client = new HttpClient();
                    client.DefaultRequestHeaders.Add("SOAPAction", "https://ptiservice.paynamics.net/pnxquery/query");
                    ...
                    xml += "<org_trxid2/>";
                    xml += "<signature>" + querySignature + "</signature>";
                    xml += "</query>";
                    xml += "</soap:Body>";
                    xml += "</soap:Envelope>";
                    HttpResponseMessage response = client.PostAsync("https://ptiservice.paynamics.net/pnxquery/queryservice.asmx", new StringContent(xml, Encoding.UTF8, "text/xml")).Result;
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        XmlNodeList node = document.GetElementsByTagName("response_code");
                        XmlNodeList nodeDescription = document.GetElementsByTagName("response_message");
                        if (node.Count > 0)
                        {
                            List<string> successCodes = new List<string> { "GR001", "GR002" };
                            string errorCode = node[1].InnerText.ToUpper();
                            string errorDescription = nodeDescription.Count > 0 ? nodeDescription[0].InnerText.ToUpper() : "";
                            if (!successCodes.Contains(node[1].InnerText.ToUpper()))
                            {
                    ...
                    if (!String.IsNullOrEmpty(orderId))
                    {
                        order = await _orderService.GetOrderByNumber(Convert.ToInt32(orderId));
                        if (order != null)
                        {
                            ...
                            await _orderService.AddPaymentColection(new PaymentCollections {
                                Active = true,
        ...
        public class PaynamicsQueryResponse
        {
